...
function isLikelySupabaseJwt(token){
  try{
    var clean = String(token || '').replace(/^Bearer\s+/i, '').trim();
    ...
    return iss.indexOf('supabase') !== -1 || ref === 'pszychsrfssqozxpwbdr' || role === 'anon' || role === 'authenticated' || role === 'service_role';
...
function isLovableAuthCaptureUrl(url){
  try{
    ...
    return host === 'api.lovable.dev' || host === 'lovable-api.com' || host === 'lovable.dev' || /\.lovable\.dev$/.test(host);
  }catch(e){ return false; }
}
...
function normalizeSkill(raw){
  ...
  const slug = String(raw.slug || raw.name || raw.handle || raw.key || raw.id || '').trim().replace(/^\/skill:/, '').replace(/\s+/g, '-');
    if (!slug || slug.length > 80) return null;
    const labelSource = raw.title || raw.label || raw.displayName || raw.display_name || raw.name || slug;
  const label = String(labelSource).replace(/^\/skill:/, '').replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
...
function notifyFound(token, projectId, force = false){
  ...
  if(normalizedToken && normalizedToken !== capturedToken){
    ...
    try { window.__lovasiriLovableToken = normalizedToken; } catch(_) {}
    try { window.__lovableAuthToken = normalizedToken; } catch(_) {}
      changed = true;
    }
  ...
  try{
    ...
    window.fetch = async function(...args){
      ...
      try{
        ...
        if(args[0] instanceof Request){
          ...
          auth = (args[0].headers && typeof args[0].headers.get === "function") ? (args[0].headers.get("Authorization") || args[0].headers.get("authorization")) : null;
            try {
              ...
              clonedReq.text().then(function(t){ try { __lovasiriCaptureUploadRequestBody(reqUrl, t); } catch(_) {} }).catch(function(){});
            }
          } catch(_) {}
          try {
            if (!opts.body && /^(PUT|POST)$/i.test(reqMethod) && __lovasiriIsSignedUploadUrl(reqUrl)) {
                const blobReq = args[0].clone();
              blobReq.blob().then(function(b){
                    try {
                      if (!b) return;
                  __lovasiriEnrichLast(reqUrl, {
                        name: null,
                    type: (b.type || (args[0].headers && args[0].headers.get && args[0].headers.get('content-type')) || null),
        ...
        try { __lovasiriPreCaptureUploadFile(reqUrl, reqMethod, reqBodyRaw, args[0]); } catch(_) {}
      }catch(e){}
      const response = await originalFetch.apply(this,args);
        inspectLovableResponse(reqUrl, response);
      try { __lovasiriCaptureUploadUrl(reqUrl, reqMethod, reqBodyText, reqBodyRaw, response); } catch(e) {}
      return response;
    };
  ...
  try{
    ...
    XMLHttpRequest.prototype.open = function(method,url){
      ...
      try { this.addEventListener('load', function(){
        try { inspectLovableText(this._lovable_url, this.responseText); } catch(e){}
        try {
          if (typeof Response !== 'undefined' && this.responseText && /\/files\/(?:generate-upload-url|upload-url)(\?|$|#)/i.test(String(this._lovable_url || ''))) {
                var headers = new Headers();
            var ct = this.getResponseHeader && this.getResponseHeader('content-type');
            if (ct) headers.set('content-type', ct);
            __lovasiriCaptureUploadUrl(this._lovable_url, this._lovable_method || 'GET', this._lovable_body_text || null, this._lovable_body_raw || null, new Response(this.responseText, { headers: headers }));
          }
        } catch(e){}
      }); } catch(e){}
    ...
    XMLHttpRequest.prototype.setRequestHeader = function(name,value){
      if(name && name.toLowerCase()==="authorization" && value && value.startsWith("Bearer ") && isLovableAuthCaptureUrl(this._lovable_url)){
...
(function setupLovasiriSpeechBridge(){
  ...
  function stop(){
    ...
    if(msg.type !== "lovasiriSpeechStart") return;
      ...
      rec.onerror = (event) => emit("error", { error: event.error || "Voice input error" });
        rec.onend = () => {
...
window.addEventListener('message', function(ev){
  ...
  if (d.type === 'lovasiriNativePromptSent' && typeof d.text === 'string' && d.text.trim()) {
    try { __lovasiriRememberNativePrompt(__lovasiriHumanPromptOnly(d.text)); } catch(e) {}
    try { __lovasiriScheduleDomRepair(); } catch(e) {}
    return;
  }
  if (d.type === 'lovasiriNativeIntercept' && typeof d.enabled === 'boolean') {
    window.__lovasiri_native_intercept = d.enabled;
    if (d.method) {
      var m = String(d.method || '').toLowerCase();
      var allowed = { v5:1, v6:1, v7:1, v8:1 };
      // Retired Methods 5–7 → same-lane Method 3/4
      if (m === 'v9' || m === 'v11') m = 'v8';
      else if (m === 'v10') m = 'v7';
      window.__lovasiri_native_method = allowed[m] ? m : 'v6';
    }
    if (d.intent) window.__lovasiri_native_intent = String(d.intent || '');
    console.log('[LovasiriIntercept] enabled =', d.enabled, 'method =', window.__lovasiri_native_method, 'intent =', window.__lovasiri_native_intent);
  }
});
...
function __lovasiriMakeBuildEventId(payload) {
  try {
    if (payload && typeof payload.build_event_id === 'string' && payload.build_event_id) return payload.build_event_id;
...
// Chat UI must show only the human prompt — never USER_REQUEST / Important to follow.
function __lovasiriHumanPromptOnly(text) {
  try {
    if (typeof window.__pkStripSteer === 'function') {
      var stripped = window.__pkStripSteer(text);
      if (stripped) return stripped;
    }
  } catch (_) {}
  var raw = String(text || '');
  if (!raw.trim()) return '';
  if (raw.indexOf('USER_REQUEST:') === -1 && raw.indexOf('Important to follow:') === -1 && raw.indexOf('[PowerKits] Treat the message above') === -1) {
    return raw.trim();
  }
  var same = raw.match(/^\s*USER_REQUEST:\s*(.+?)(?:\n\s*Important to follow:|\n\s*\[PowerKits\]|$)/is);
  if (same && same[1] && same[1].trim()) return same[1].trim();
  var multi = raw.match(/USER_REQUEST:\s*\n([\s\S]*?)(?:\n\s*Important to follow:|\n\s*\[PowerKits\])/i);
  if (multi && multi[1] && multi[1].trim()) return multi[1].trim();
  return raw.split(/\n\s*Important to follow:/i)[0].replace(/^\s*USER_REQUEST:\s*/i, '').trim();
}

/**
 * Credit-free rewrite lives on PowerKits server.
 * Thin stub: ask SW → /api/public/ext/rewrite-chat (HMAC session + usable license).
 * Unlicensed / revoked / expired → null (passthrough native Lovable chat).
 */
function __lovasiriRewriteBodyAsync(bodyStr) {
  return new Promise(function (resolve) {
    try {
      ...
      try {
        nativeActive = document.documentElement.getAttribute('data-ql-native-chat-active') === '1';
      } catch (_) {}
      if (!nativeActive) {
        resolve(null);
        return;
      ...
      var domMethod = '';
      try { domMethod = document.documentElement.getAttribute('data-lovasiri-method') || ''; } catch (_) {}
      var raw = String(domMethod || window.__lovasiri_native_method || 'v6').toLowerCase();
      var allowed = { v5:1, v6:1, v7:1, v8:1 };
      if (raw === 'v9' || raw === 'v11') raw = 'v8';
      else if (raw === 'v10') raw = 'v7';
      var method = allowed[raw] ? raw : 'v6';
      var id = 'pkrw-' + Math.random().toString(36).slice(2) + '-' + Date.now();
      var done = false;
      var finish = function (body, human) {
        if (done) return;
        done = true;
        try { window.removeEventListener('message', onMsg); } catch (_) {}
        if (body) {
          try {
            if (human) __lovasiriRememberNativePrompt(human);
            else {
              try {
                var p = JSON.parse(bodyStr);
                var raw = (p && (p.message || p.text || p.content)) || '';
                __lovasiriRememberNativePrompt(__lovasiriHumanPromptOnly(raw));
              } catch (_) {}
            }
          } catch (_) {}
          try { __lovasiriScheduleDomRepair(); } catch (_) {}
          console.log('[LovasiriIntercept] ✅ server rewrite applied');
        }
        resolve(body || null);
      };
      var onMsg = function (event) {
        if (event.source !== window) return;
        var msg = event.data || {};
        if (msg.source !== 'PK_REWRITE_BRIDGE' || msg.id !== id) return;
        if (msg.ok && msg.body) finish(String(msg.body), msg.human || '');
        else finish(null);
      };
      window.addEventListener('message', onMsg);
      window.postMessage({
        source: 'PK_REWRITE_REQUEST',
        id: id,
        type: 'pkRewriteChat',
        chatBody: bodyStr,
        method: method,
...
function __lovasiriRememberNativePrompt(text) {
  try {
    var promptText = __lovasiriHumanPromptOnly(text);
    ...
    try { sessionStorage.setItem('__lovasiri_last_native_prompt', JSON.stringify(item)); } catch(_) {}
  } catch(e) {}
}
...
function __lovasiriScrubUiValue(value) {
  try {
    if (typeof value === 'string') {
      if (!/(fix_error|security_scan|deploy_error|security_finding_fix|security_fix_all|error_try_to_fix|seo_scan|message_intent_metadata|fix_error_metadata|security_scan_metadata|deploy_error_metadata|security_finding_fix_metadata|security_fix_all_metadata|error_try_to_fix_metadata|seo_scan_metadata|runtime_errors|build_event_id)/.test(value)) return value;
      var trimmed = value.trim();
      if (!/^[\[{]/.test(trimmed)) return value;
      try { return JSON.stringify(__lovasiriScrubUiValue(JSON.parse(value))); } catch(e) { return value; }
    }
    ...
    if (!__lovasiriVisiblePrompt && __lovasiriLooksLikeCurrentFixError(value)) __lovasiriVisiblePrompt = __lovasiriGetRecentNativePrompt();
    ...
    Object.keys(value).forEach(function(key){
      if (
        ...
        key === 'deploy_error_metadata' ||
        key === 'security_finding_fix_metadata' ||
        key === 'security_fix_all_metadata' ||
        key === 'error_try_to_fix_metadata' ||
        key === 'seo_scan_metadata' ||
        key === 'runtime_errors' ||
        key === 'error_ids' ||
        key === 'build_event_id'
      ) {
      ...
      if ((key === 'intent' || key === 'message_intent') && (value[key] === 'fix_error' || value[key] === 'security_scan' || value[key] === 'deploy_error' || value[key] === 'security_finding_fix' || value[key] === 'security_fix_all' || value[key] === 'error_try_to_fix' || value[key] === 'seo_scan')) {
        delete value[key];
        return;
      }
...
function __lovasiriExtractFixErrorPrompt(value) {
  try {
    ...
    var directSecMessage = sec && (sec.error_message || sec.errorMessage || sec.message || sec.text || sec.prompt || sec.user_message || sec.userMessage || sec.display_text || sec.displayText);
    if (typeof directSecMessage === 'string' && directSecMessage.trim() && !__lovasiriIsFixMethodLabel(directSecMessage)) {
      return __lovasiriHumanPromptOnly(directSecMessage);
    }
    var fix = meta && (meta.fix_error_metadata || meta.fixErrorMetadata);
    var deploy = meta && (meta.deploy_error_metadata || meta.deployErrorMetadata);
    var shell = fix || deploy;
    var directFixMessage = shell && (shell.error_message || shell.errorMessage || shell.message || shell.text || shell.prompt || shell.user_message || shell.userMessage);
    if (typeof directFixMessage === 'string' && directFixMessage.trim() && !__lovasiriIsFixMethodLabel(directFixMessage)) {
      return __lovasiriHumanPromptOnly(directFixMessage);
    }
    var errors = shell && Array.isArray(shell.errors) ? shell.errors : [];
    for (var i = 0; i < errors.length; i++) {
      var msg = errors[i] && (errors[i].error_message || errors[i].errorMessage || errors[i].message || errors[i].text || errors[i].prompt || errors[i].user_message || errors[i].userMessage);
      if (typeof msg === 'string' && msg.trim() && !__lovasiriIsFixMethodLabel(msg)) return __lovasiriHumanPromptOnly(msg);
    }
  } catch(e) {}
  return '';
}
...
function __lovasiriIsFixMethodLabel(v) {
  try {
    ...
    return t === 'fix build error' ||
      t === 'corrigir erro de build' ||
      t === 'corrigir erro de buid' ||
      t === 'fix deploy error' ||
      t === 'corrigir erro de deploy' ||
      t === 'security scan' ||
      t === 'scan de segurança' ||
      t === 'security_scan' ||
      t === 'fix_error' ||
      t === 'deploy_error' ||
      t === 'security_finding_fix' ||
      t === 'security_fix_all' ||
      t === 'error_try_to_fix' ||
      t === 'seo_scan' ||
      ...
      t === 'seo scan' ||
      t === 'lovable powerkits' ||
      t === '〽️ enviado por lovasiri' ||
      t === '〽️ enviado por lovasiri2' ||
      t === 'enviado por lovasiri' ||
      t === 'enviado por lovasiri2';
  } catch(e) { return false; }
}

function __lovasiriLooksLikeSteeredPrompt(v) {
  try {
    if (typeof v !== 'string') return false;
    return /USER_REQUEST:|Important to follow:|\[PowerKits\] Treat the message above/i.test(v);
  } catch (e) {
    return false;
  }
}
...
function __lovasiriShouldReplaceVisibleValue(v) {
  return __lovasiriIsBlankVisibleValue(v) || __lovasiriIsFixMethodLabel(v) || __lovasiriLooksLikeSteeredPrompt(v);
}
...
function __lovasiriLooksLikeCurrentFixError(obj) {
  try {
    if (!obj || typeof obj !== 'object') return false;
    if (/^(fix_error|security_scan|deploy_error|security_finding_fix|security_fix_all|error_try_to_fix|seo_scan)$/.test(String(obj.intent||'')) || /^(fix_error|security_scan|deploy_error|security_finding_fix|security_fix_all|error_try_to_fix|seo_scan)$/.test(String(obj.message_intent||''))) return true;
    if (obj.message_intent_metadata || obj.messageIntentMetadata || obj.fix_error_metadata || obj.fixErrorMetadata || obj.security_scan_metadata || obj.securityScanMetadata || obj.deploy_error_metadata || obj.deployErrorMetadata) return true;
  } catch(e) {}
  return false;
}
...
function __lovasiriPromotePromptToVisibleFields(obj, prompt, forceCreate) {
  try {
    ...
    var keys = ['message', 'text', 'content', 'body', 'prompt', 'user_message', 'userMessage', 'display_text', 'displayText', 'value', 'markdown'];
    ...
    if (Array.isArray(obj.segments)) {
      if (obj.segments.length === 0) {
        ...
        for (var s = 0; s < obj.segments.length; s++) {
          if (obj.segments[s] && __lovasiriShouldReplaceVisibleValue(obj.segments[s].text)) obj.segments[s].text = prompt;
        }
      }
    }
    ...
    Object.keys(obj).forEach(function(childKey){
      if (childKey === 'message_intent_metadata' || childKey === 'fix_error_metadata' || childKey === 'security_scan_metadata' || childKey === 'deploy_error_metadata') return;
      var child = obj[childKey];
      if (child && typeof child === 'object') {
        if (Array.isArray(child)) {
          child.forEach(function(item){ if (item && typeof item === 'object') __lovasiriPromotePromptToVisibleFields(item, prompt, false); });
        } else {
...
function __lovasiriCleanJsonText(text) {
  try {
    ...
    return String(text || '')
      .replace(/("intent"\s*:\s*)"(?:fix_error|security_scan|deploy_error|security_finding_fix|security_fix_all|error_try_to_fix|seo_scan)"/g, '$1null')
      .replace(/("message_intent"\s*:\s*)"(?:fix_error|security_scan|deploy_error|security_finding_fix|security_fix_all|error_try_to_fix|seo_scan)"/g, '$1null')
...
function __lovasiriSanitizeResponseForUi(response) {
  try {
    ...
    var carry = '';
    var stream = response.body.pipeThrough(new TransformStream({
...
function __lovasiriGetRecentNativePrompt() {
  try {
    ...
    if (!last || !last.text) {
      try { last = JSON.parse(sessionStorage.getItem('__lovasiri_last_native_prompt') || 'null'); } catch(_) {}
    }
...
function __lovasiriIsInsideComposer(el) {
  try {
    return !!(el && el.closest && el.closest('form#chat-input, textarea, [contenteditable="true"], [data-testid*="chat-input" i], [class*="composer" i]'));
  } catch(e) { return false; }
}
...
function __lovasiriReplaceNodeTextWithPrompt(node, prompt) {
  try {
    ...
    while (el && steps < 7) {
      ...
      if (el.getAttribute && (el.getAttribute('role') === 'article' || el.getAttribute('data-message-id') || /message/i.test(String(el.getAttribute('data-testid') || '') + ' ' + String(el.className || '')))) break;
      el = el.parentElement;
      steps++;
    }
...
function __lovasiriScheduleDomRepair() {
  try {
    [80, 250, 700, 1500, 3000, 6000].forEach(function(ms){ setTimeout(__lovasiriHideFixDecorationsOnly, ms); });
  } catch(e) {}
}

function __lovasiriTextIsExact(el, labels) {
  try {
    var t = String((el && el.textContent) || '').replace(/\s+/g, ' ').trim().toLowerCase();
...
function __lovasiriHideExactElementFromTextNode(node, labels, replaceWithPrompt) {
  try {
    ...
    while (el && steps < 4) {
      ...
      if (el.getAttribute && (el.getAttribute('role') === 'article' || el.getAttribute('data-message-id'))) { inMessage = true; break; }
      el = el.parentElement;
      steps++;
    }
...
function __lovasiriInstantSwapLabelNode(node) {
  try {
    ...
    if (!raw) return false;
    var securityLabels = ['security scan', 'scan de segurança', 'scan de seguranca', 'security_scan'];
    var fixLabels = ['fix build error', 'corrigir erro de build', 'corrigir erro de buid', 'fix deploy error', 'corrigir erro de deploy', 'deploy_error', 'security finding fix', 'security fix all', 'error try to fix', 'seo scan', 'security_finding_fix', 'security_fix_all', 'error_try_to_fix', 'seo_scan'];
    ...
    var lovLabel = '❤️ Sent by Lovable';
    ...
    try { node.nodeValue = lovLabel; } catch(_) {}
...
function __lovasiriHideFixDecorationsOnly() {
  try {
    ...
    while ((node = walker.nextNode())) {
      ...
      } else if (recentPrompt && __lovasiriLooksLikeSteeredPrompt(rawValue) && !__lovasiriIsInsideComposer(node.parentElement)) {
...
(function wrapChatIntercept(){
  try {
    ...
    window.fetch = async function(input, init) {
      ...
      } catch(e) { console.warn('[LovasiriIntercept] fetch wrap err', e); }
    ...
    XMLHttpRequest.prototype.open = function(method, url) {
      this.__lov_method = method; this.__lov_url = url;
    ...
    XMLHttpRequest.prototype.send = function(body) {
      ...
      try {
        ...
        try { __lovasiriPreCaptureUploadFile(this.__lov_url || this._lovable_url, this.__lov_method || this._lovable_method, body, null); } catch(e) {}
        if (window.__lovasiri_native_intercept && __lovasiriIsChatPost(this.__lov_url, this.__lov_method) && typeof body === 'string') {
          // Hold send until server rewrite answers (or denies → native body).
          __lovasiriRewriteBodyAsync(body).then(function (rw) {
            try {
              return origSend.call(xhr, rw || body);
            } catch (_) {
    ...
    if (document && document.documentElement) {
      ...
      var domClean = function(){
        ...
        if (!/\/projects\/[0-9a-fA-F-]{36}/.test(location.pathname)) return;
        try { __lovasiriRestoreHiddenUi(); } catch(e){}
        try { __lovasiriHideFixDecorationsOnly(); } catch(e){}
      };
      ...
      setInterval(function(){ if (!document.hidden) scheduleDomClean(); }, 3000);
      try {
        // childList+subtree apenas — sem characterData/attributes (evita disparo em cada re-render).
        new MutationObserver(scheduleDomClean).observe(document.body || document.documentElement, { childList: true, subtree: true });
      } catch(e) {}

      // [v13.87] Observer instantâneo dedicado ao rótulo do método.
      // Roda síncrono a cada mutação de texto/nó — sem rAF, sem debounce —
      // para que "Corrigir erro de build" / "Security scan" nunca chegue a
      ...
      try {
        ...
        new MutationObserver(instantLabelSwap).observe(
  ...
  } catch(e) { console.warn('[LovasiriIntercept] falha instalar', e); }
})();
...
function __lovasiriGuessMimeFromName(name, currentType) {
  ...
  var map = {
    png: 'image/png', jpg: 'image/jpeg', jpeg: 'image/jpeg', webp: 'image/webp', gif: 'image/gif', svg: 'image/svg+xml', bmp: 'image/bmp', avif: 'image/avif',
    txt: 'text/plain', md: 'text/markdown', csv: 'text/csv', tsv: 'text/tab-separated-values',
    json: 'application/json', env: 'text/plain', log: 'text/plain', xml: 'application/xml',
    yaml: 'application/yaml', yml: 'application/yaml', zip: 'application/zip', rar: 'application/vnd.rar',
...
function __lovasiriCaptureUploadRequestBody(url, text) {
  try {
    ...
    var body = null;
    try { body = JSON.parse(String(text || '')); } catch(_) { return; }
...
function __lovasiriIsSignedUploadUrl(url) {
  return (
    ...
    /amazonaws\.com|storage\.googleapis\.com|blob\.core\.windows\.net|r2\.cloudflarestorage\.com/i.test(String(url || ''))
  );
}
...
function __lovasiriCaptureUploadUrl(url, method, reqBodyText, reqBodyRaw, response) {
  try {
    ...
    var reqBody = null;
    if (reqBodyText) { try { reqBody = JSON.parse(reqBodyText); } catch(_) {} }
    ...
    response.clone().json().then(function(j){
      try {
        ...
        if (recent && recent.name && (pickName === 'anexo' || /^file(?:\.|$)/i.test(String(pickName)) || /^attachment(?:\.|$)/i.test(String(pickName)))) {
          pickName = recent.name;
          pickType = __lovasiriGuessMimeFromName(pickName, recent.type || pickType) || pickType;
        }
        var pickSize = (formMeta && formMeta.size)
        ...
        console.log('[LovaSiri] 📎 Native upload captured:', meta.file_name, meta.file_id);
